...
/* Inicializar variable global con defaults de fábrica */
...
esp_err_t config_init(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGI(TAG, "NVS vacío. Usando defaults de fábrica de node_config.h");
        return ESP_OK;
    }
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGI(TAG, "Cargado temp_threshold=%.2f °C desde NVS", g_config.temp_threshold_c);
    }

    err = nvs_get_u32(nvs, "hum_th", &u32_val);
    if (err == ESP_OK) {
        g_config.hum_threshold_pct = (uint8_t)u32_val;
        ESP_LOGI(TAG, "Cargado hum_threshold=%u %% desde NVS", (uint8_t)u32_val);
    }
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGI(TAG, "Cargado heartbeat=%u s desde NVS", u16_val);
    }
...
esp_err_t config_save(void) {
    ...
    err = nvs_commit(nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_commit falló: %s", esp_err_to_name(err));
    } else {

...
void app_main(void)
{
    ...
    /* ── Loop: leer → push si umbral → io_process → dormir ── */
    float last_push_temp = TEMP_BASELINE;
    uint8_t last_push_hum = HUM_BASELINE;
    TickType_t last_push_tick = xTaskGetTickCount();

    ESP_LOGI(TAG, "Loop: leer cada %lums → push si |Δ|>umbral → io → sleep",
    ...
    while (1) {
        ...
        /* Push si temperatura cambio mas que umbral */
        if (fabsf(lectura.temperatura_c - last_push_temp) > g_config.temp_threshold_c) {
            last_push_temp = lectura.temperatura_c;
            push_telemetry(lectura.temperatura_c, lectura.humedad_pct,
                           3100, -65,
                           (uint32_t)(now * portTICK_PERIOD_MS / 1000));
            pushed = true;
        }

        /* Push si humedad cambio mas que umbral */
        int16_t dh = abs((int16_t)lectura.humedad_pct - (int16_t)last_push_hum);
        if (dh > g_config.hum_threshold_pct) {
            last_push_hum = lectura.humedad_pct;
            if (!pushed) {
                push_telemetry(lectura.temperatura_c, lectura.humedad_pct,
                               3100, -65,
                               (uint32_t)(now * portTICK_PERIOD_MS / 1000));
        ...
        /* Heartbeat: push aunque no haya cambio */
        if (!pushed && (now - last_push_tick) >= pdMS_TO_TICKS(g_config.heartbeat_s * 1000)) {
            last_push_temp = lectura.temperatura_c;
            last_push_hum  = lectura.humedad_pct;
            push_telemetry(lectura.temperatura_c, lectura.humedad_pct,
                           3100, -65,
        ...
        if (pushed) last_push_tick = now;

        /* Procesar peticiones CoAP entrantes (GET /env, PUT /config, etc.) */

...
esp_err_t config_init(void)
{
    ...
    g_config.hum_threshold_pct  = HUM_THRESHOLD_PCT_DEFAULT;
    ...
    /* ── Sobrescribir con NVS si existe ── */
    nvs_handle_t nvs;
    ...
    if (nvs_get_u32(nvs, "hum_th",   &u32) == ESP_OK) g_config.hum_threshold_pct  = (uint8_t)u32;
    ...
    ESP_LOGI(TAG, "Config cargada de NVS: T>%.1fC H>%d%% HB=%ds SI=%lums Lat=%.6f Lng=%.6f",

...
{
    g_config.hum_threshold      = HUM_THRESHOLD_DEFAULT;
    ...
    uint32_t u32;
    if (nvs_get_u32(nvs, "hum_th",    &u32) == ESP_OK) g_config.hum_threshold = (float)u32 / 100.0f;
    ...
    nvs_close(nvs);
    ESP_LOGI(TAG, "Config: H>%.1f%% HB=%ds SI=%lums", (double)g_config.hum_threshold,
...
esp_err_t config_save(void)
{
    ...
    nvs_set_u32(nvs, "hum_th",    (uint32_t)(g_config.hum_threshold * 100.0f));

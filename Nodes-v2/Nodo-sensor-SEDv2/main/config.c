...
{
    g_config.luz_threshold      = LUZ_THRESHOLD_DEFAULT;
    ...
    uint32_t u32;
    if (nvs_get_u32(nvs, "luz_th",    &u32) == ESP_OK) g_config.luz_threshold = (float)u32 / 100.0f;
    ...
    nvs_close(nvs);
    ESP_LOGI(TAG, "Config: L>%.1f%% HB=%ds SI=%lums", (double)g_config.luz_threshold,
...
esp_err_t config_save(void)
{
    ...
    nvs_set_u32(nvs, "luz_th",   (uint32_t)(g_config.luz_threshold * 100.0f));

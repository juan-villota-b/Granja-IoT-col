...
class MQTTPublisher:
 def __init__(self,cfg):
  ...
  self._at=c.get("attributes_topic","v1/gateway/attributes");self._dt=c.get("disconnect_topic","v1/gateway/disconnect")
 ...
 def attributes(self,dev,vals):
  if not self._connected:return
  self._cl.publish(self._at,json.dumps({dev:vals}),qos=1)

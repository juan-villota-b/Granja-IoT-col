# Nodo CoAP SED — ESP32-C6 Thread Ultra Bajo Consumo

Firmware para **ESP32-C6 como Sleepy End Device (SED)**: nodo Thread cliente CoAP que solo transmite cuando hay cambios significativos, con consumo **~15 µA promedio** (~12 años con batería 2000mAh).

Es idéntico en funcionalidad a `nodo-coap-sed/`. Esta carpeta es una copia independiente para otro nodo físico con configuración propia (coordenadas, ID, umbrales, etc.).

## Diferencia con nodo-coap-sed

| Carpeta | Nodo físico | ID | Coordenadas |
|---------|-------------|----|-------------|
| `nodo-coap-sed/` | NODO-TH-AUTO-1 | `NODO-TH-AUTO` | 5.031303, -75.468862 |
| **`nodo-coap-sed-2/`** | **NODO-TH-AUTO-2** | **`NODO-TH-AUTO-2`** | **(varía)** |

Ambos firmware son estructuralmente iguales. La configuración específica (ID, zona, coordenadas, umbrales) se define en `main/node_config.h` de cada carpeta.
...
El radio 802.15.4 duerme entre transmisiones. El nodo no rutea paquetes para otros.

**Ahorro:** ~20 mA (FTD) → ~0 mA idle (SED).
...
Cuando el procesador no tiene tareas que ejecutar (durante `vTaskDelay(30000)`), FreeRTOS entra en light sleep automático.
...
```c
#define NODE_ID             "NODO-TH-AUTO-2"  // ← DIFERENTE de nodo-coap-sed
...
```
I (394) nodo: === Nodo SED [NODO-TH-AUTO-2] ===
